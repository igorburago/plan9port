...
iconinit(void)
{
	...
	if(tagcols[BACK] == nil){
		tagcols[BACK] = allocimage(display, Rect(0,0,1,1), screen->chan, 1, 0x3A2E23FF);
		tagcols[TEXT] = allocimage(display, Rect(0,0,1,1), screen->chan, 1, 0xEBCAA0FF);
		...
		textcols[BORD] = allocimage(display, Rect(0,0,1,1), screen->chan, 1, 0x4C3D2EFF);
	}
