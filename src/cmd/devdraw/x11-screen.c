...
runxevent(XEvent *xev)
{
	...
	/*
	 * Ignore events generated by XWarpPointer() in rpc_setmouse().
	 * Although XEvent.serial is ulong, it stores only the 16 least
	 * significant bits of the X11 request serial number.
	 */
	if(u16sub(xev->serial, _x.mwarpserial) < _x.mwarpcount)
		return;
	_x.mwarpcount = 0;
...
static uint
xpinserial(XDisplay *d)
{
	uint serial;

	serial = NextRequest(d);
	XNoOp(d);
	return serial;
}
...
rpc_setmouse(Client *client, Point p)
{
	...
	xlock();
	_x.mwarpserial = xpinserial(_x.display);
	XWarpPointer(_x.display, None, w->drawable, 0, 0, 0, 0, p.x, p.y);
	_x.mwarpcount = u16sub(xpinserial(_x.display), _x.mwarpserial);
	XFlush(_x.display);
	xunlock();
}
